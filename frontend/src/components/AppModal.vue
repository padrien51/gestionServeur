...
<template>
  <transition name="modal-fade">
    <div v-if="state.isOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <!-- Modal Box -->
      <div class="relative bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl w-full max-w-md overflow-hidden flex flex-col transform transition-all">
        
        <!-- Header -->
        <div class="px-6 py-4 border-b border-slate-800 flex justify-between items-center" 
             :class="state.type === 'confirm' ? 'bg-orange-950/30' : 'bg-slate-800/50'">
          ...
          <button @click="close(false)" class="text-slate-400 hover:text-slate-200 transition-colors">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
          </button>
        </div>
        
        <!-- Body -->
        <div class="px-6 py-5 text-slate-300 text-sm whitespace-pre-line leading-relaxed">
          {{ state.message }}
        </div>
        ...
        <div class="px-6 py-4 border-t border-slate-800 bg-slate-900/50 flex justify-end gap-3">
          <template v-if="state.type === 'confirm'">
            <button @click="close(false)" class="px-4 py-2 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-lg text-sm font-medium transition-colors border border-slate-700">
              Annuler
            </button>
