...
<template>
  ...
  >
    <div v-if="containerId" class="fixed inset-0 z-50 flex flex-col justify-end pointer-events-none">
      ...
      <!-- Bottom Sheet -->
      <div class="relative bg-slate-900 w-full h-[80vh] sm:h-[90vh] rounded-t-3xl shadow-2xl flex flex-col pointer-events-auto border-t border-slate-700">
        ...
        <div class="px-6 py-4 flex justify-between items-center border-b border-slate-800">
          <div>
            ...
            <p class="text-sm text-slate-400 font-mono">{{ containerName }}</p>
          </div>
        ...
        <!-- Toolbar -->
        <div class="px-6 py-2 bg-slate-800/50 flex justify-between items-center text-sm border-b border-slate-800">
          ...
          <button 
            ...
            :class="isAutoScroll ? 'bg-teal-500/20 text-teal-400' : 'bg-slate-700 text-slate-400'"
          >
        ...
        <div 
          ref="logsContainer"
          @scroll="handleScroll"
          class="flex-1 overflow-y-auto bg-[#0d1117] p-4 font-mono text-xs sm:text-sm text-slate-300 space-y-1"
        >
