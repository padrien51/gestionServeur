...
<template>
  <div class="p-4 space-y-6">
    ...
    <div class="bg-slate-800 rounded-2xl border border-slate-700 overflow-hidden shadow-xl p-6 space-y-8">
      
      <!-- Modifier l'email -->
      <div>
        <h3 class="text-lg font-semibold text-slate-200 border-b border-slate-700 pb-2 mb-4 flex items-center">
        ...
        <form @submit.prevent="handleChangeEmail" class="flex flex-col md:flex-row gap-4 items-end">
          <div class="flex-1 w-full">
            <label class="block text-sm font-medium text-slate-300 mb-2">Nouvelle adresse email</label>
            <input 
              v-model="newEmail" 
              type="email" 
